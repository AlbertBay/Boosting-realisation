...
class Booster:
    ...
    def fit(self, X, y):
        ...
        for estimator_n in range(1, self.n_estimators):
            self._fit_base_estimator(X, y, ay)
            ay += self.lr * self.weights[-1] * self.estimators[-1].predict(X[:, self.indices[-1]])
    ...
    def predict(self, X):
        ...
        for estimator_n in range(1, self.n_estimators):
            predictions += self.lr * self.weights[estimator_n] * self.estimators[estimator_n].predict(
                X[:, self.indices[estimator_n]])
...
class GradientBoostingClassifier(Booster):
    ...
    def _loss(self, y_true, y_pred):
        ...
        return -y_true.dot(y_pred) + np.sum(np.logaddexp(0, y_pred))
    ...
    def _fit_base_estimator(self, X, y, predictions):
        ...
        base_est = self.base_estimator(**self.params)
        self.estimators.append(base_est.fit(x, -grad))
        self.indices.append(ind_features)

        weight = minimize_scalar(lambda alpha: self._loss(y, predictions + alpha*base_est.predict(X[:, ind_features]))).x
